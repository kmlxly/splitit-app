...
function SplitItContent() {
    ...
    useEffect(() => {
        const initApp = async () => {
            ...
            if (currentUser) {
                ...
                try {
                    ...
                    if (joinSessionId) {
                        try {
                            ...
                            alert("Successfully joined friend's session!");
                        } catch (e) {
                ...
                if (joinSessionId) {
                    alert("Please sign in (tap the user icon above) to join your friend's session!");
                }
    ...
    const deleteSession = async (sid: string) => {
        ...
        try {
            ...
            alert(targetSession.isShared
                ? "Could not leave event. Please try again."
    ...
    const deletePerson = (id: string) => {
        if (bills.some(b => b.paidBy === id || b.details.some(d => d.personId === id))) {
            alert("Cannot delete: this person is already assigned to a bill."); return;
        }
    ...
    const saveBill = async () => {
        ...
        if (!billTitle.trim() || !billTotal || !activePayerId) {
            if (people.length === 0) {
                alert("Please add at least one member to the crew first!");
    ...
    const copyWhatsAppSummary = () => {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            ...
            try {
                ...
                alert("Clipboard copy not permitted. Please take a manual screenshot.");
            }
    ...
    const handleScanReceipt = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (Array.isArray(expandedItems) && expandedItems.length > 0) {
                ...
                alert("AI could not detect any line items on this receipt.");
            }

        } catch (err: any) { console.error(err); alert(`Scan failed: ${err.message}`); }
        setIsScanning(false);
    };
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, pid: string) => {
        ...
        if (file) {
            if (file.size > 5000000) { alert("File is too large (> 5MB)."); return; }
            const reader = new FileReader();
            reader.onloadend = () => {
    ...
    const forceSync = async () => {
        ...
        if (confirm("Force sync to Cloud now?") === false) return;
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="grid grid-cols-2 gap-3">
                                <div className={`${cardStyle} p-4 flex flex-col items-center justify-center ${shadowStyle}`}>
                                    <span className="text-[10px] uppercase font-bold opacity-60 mb-1">Total Spent</span>
                            ...
                            <section data-guide="splitit-people" className={`${cardStyle} p-5 ${darkMode ? "" : "bg-violet-100"} ${shadowStyle}`}>
                                <div className="flex items-center justify-between mb-4"><h2 className="text-sm font-black uppercase tracking-widest flex items-center gap-2"><Users size={16} /> Members / Crew</h2></div>
                                ...
                                <div className="space-y-4">
                                    {bills.length === 0 ? (
                                        ...
                                        <>
                                                        ...
                                                        <div className={`text-sm border-t-2 p-5 space-y-3 ${darkMode ? "border-white bg-[#1a1a1a]" : "border-black bg-gray-50"}`}>
                                                            ...
                                                            {bill.type === "ITEMIZED" && bill.menuItems && bill.menuItems.length > 0 && (
                                                                <div className="mb-4 pb-4 border-b border-dashed border-current border-opacity-20">
                                                                    <p className="text-[10px] font-bold uppercase opacity-50 mb-2">Ordered Items:</p>
                                            ...
                                            <button onClick={() => { resetForm(); setMode("FORM") }} className={`w-full py-4 ${buttonBase} ${darkMode ? "bg-[#333]" : "bg-white"} mt-4 text-sm uppercase tracking-wider`}><Plus size={18} /> Add Another Bill</button>
                                        </>
                                    )}
                                </div>
                            </section>
                            ...
                            {bills.length > 0 && (
                                <section>
                                    ...
                                    <div className={`${cardStyle} p-6 ${shadowStyle} ${darkMode ? "bg-[#222]" : "bg-lime-200"}`}>
                                        <div className="mb-6 text-center border-b-2 border-current border-opacity-10 pb-4"><span className="text-[10px] uppercase font-bold opacity-60">Total Spent</span><div className="text-3xl font-mono font-black mt-1">{currency}{totalSpent.toFixed(2)}</div></div>
                            ...
                            <div className={`flex-1 space-y-8 overflow-y-auto pb-32 px-1 ${darkMode ? "scrollbar-thumb-white" : "scrollbar-thumb-black"} scrollbar-thin`}>
                                <div className={`${cardStyle} p-5 space-y-5 ${darkMode ? "bg-[#1E1E1E]" : "bg-white"} ${shadowStyle}`}>
                                    ...
                                    <div className="space-y-2 p-3 rounded-xl border-2 border-dashed border-current border-opacity-30 bg-current/5">
                                            ...
                                            <div className="relative group w-full">
                                                ...
                                                >
                                                    <optgroup label="Session Base">
                                                        <option value={currency}>{currency} (Session Base)</option>
                                                    </optgroup>
                                        ...
                                        {/* Final Total - ReadOnly if Foreign, Editable if Base */}
                                        <div>
                                            <label className="text-[9px] font-bold opacity-50 block mt-1">
                                                {formCurrency !== currency ? `Auto-Convert (${currency})` : `Receipt Total (${currency})`}
                                            </label>
                                            <input
                                                type="number"
                                                value={billTotal}
                                                onChange={e => setBillTotal(e.target.value)}
                                                readOnly={formCurrency !== currency}
                                                placeholder="0.00"
                                    ...
                                    <div className="space-y-2"><label className="text-xs uppercase font-black tracking-wider opacity-70">Payer</label><div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">{people.map(p => { const isSelected = (payerId || people[0]?.id) === p.id; return (<button key={p.id} onClick={() => setPayerId(p.id)} className={`px-4 py-3 rounded-xl text-sm font-bold border-2 whitespace-nowrap transition-all ${isSelected ? (darkMode ? "bg-white text-black border-white shadow-[2px_2px_0px_0px_#ffffff50]" : "bg-black text-white border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] translate-x-[-2px] translate-y-[-2px]") : (darkMode ? "border-[#444] text-gray-400 hover:border-white" : "border-gray-300 text-gray-500 hover:border-black hover:text-black hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]")}`}>{p.name}</button>); })}</div></div>
                                </div>

                                <div className="grid grid-cols-2 gap-3">
                                    <button onClick={() => setBillType("EQUAL")} className={`p-4 rounded-xl border-2 text-sm font-black uppercase tracking-wider transition-all flex flex-col items-center gap-2 ${billType === "EQUAL" ? (darkMode ? "border-green-400 bg-green-400/20 text-green-400" : "border-black bg-green-300 text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] translate-x-[-2px] translate-y-[-2px]") : (darkMode ? "border-[#444] opacity-50 hover:opacity-100" : "border-black bg-white opacity-50 hover:opacity-100 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]")}`}><span className="text-2xl">🍰</span> EQUAL SPLIT</button>
                                    <button onClick={() => setBillType("ITEMIZED")} className={`p-4 rounded-xl border-2 text-sm font-black uppercase tracking-wider transition-all flex flex-col items-center gap-2 ${billType === "ITEMIZED" ? (darkMode ? "border-blue-400 bg-blue-400/20 text-blue-400" : "border-black bg-blue-300 text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] translate-x-[-2px] translate-y-[-2px]") : (darkMode ? "border-[#444] opacity-50 hover:opacity-100" : "border-black bg-white opacity-50 hover:opacity-100 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]")}`}><span className="text-2xl">🧾</span> ITEMIZED</button>
                                </div>
                                    ...
                                    <div className="space-y-6 animate-in fade-in">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <button onClick={() => setShowScanMethodModal(true)} className={`w-full py-3 rounded-xl border-2 flex items-center justify-center gap-2 font-black uppercase text-xs transition-all active:scale-95 ${darkMode ? "bg-indigo-500 text-white border-indigo-400" : "bg-indigo-100 text-indigo-700 border-indigo-300 hover:bg-indigo-200"}`}>
                                                <Camera size={16} /> Scan Receipt (Auto-Fill)
                                            </button>
                                            <div className="flex items-center gap-2 opacity-40">
                                                <div className="h-[1px] bg-current flex-1"></div>
                                                <span className="text-[9px] font-bold uppercase">OR MANUAL</span>
                                                <div className="h-[1px] bg-current flex-1"></div>
                                            </div>
                                        </div>
                                        ...
                                        <div className={`${cardStyle} p-4 space-y-4 ${darkMode ? "bg-[#1E1E1E]" : "bg-white"} ${shadowStyle} overflow-hidden transition-all duration-300 relative`}>
                                            ...
                                            <div className="flex gap-3">
                                                <div className="flex-[2] space-y-1">
                                                    <label className="text-[9px] font-bold uppercase opacity-50 ml-1">Item Name</label>
                                                    <input value={newItemName} onChange={e => setNewItemName(e.target.value)} placeholder="e.g. Fried Rice" className={`${inputStyle} text-sm py-2`} />
                                                </div>
                                                <div className="flex-1 space-y-1 relative">
                                                    <label className="text-[9px] font-bold uppercase opacity-50 ml-1">Price</label>
                                            ...
                                            <div className="pt-2 border-t border-dashed border-current border-opacity-20">
                                                <div className="flex justify-between items-center mb-2">
                                                    <label className="flex items-center gap-2 text-[10px] font-bold cursor-pointer opacity-70 hover:opacity-100"><input type="checkbox" checked={isMultiSelectMode} onChange={toggleMultiSelect} className="accent-blue-500 w-3 h-3 rounded-sm" />Shared by multiple people?</label>
                                        ...
                                        <div className={`${cardStyle} p-4 space-y-4 ${darkMode ? "bg-[#1E1E1E]" : "bg-violet-100"} ${shadowStyle}`}>
                                            <div className="flex gap-3">
                                                <div className="flex-1 space-y-1"><label className="text-[9px] uppercase font-black opacity-70 flex items-center gap-1"><Bike size={12} /> Fixed Fee / Delivery</label><input type="number" placeholder="0.00" value={miscFee} onChange={e => setMiscFee(e.target.value)} className={`${inputStyle} py-2 font-mono text-sm`} /></div>
                                        ...
                                        <div className={`p-4 border-2 rounded-xl ${shadowStyle} ${taxGap > 0.05 ? (darkMode ? "border-blue-400 bg-blue-400/10" : "border-black bg-blue-200") : taxGap < -0.05 ? (darkMode ? "border-red-400 bg-red-400/10" : "border-black bg-red-200") : (darkMode ? "border-green-400 bg-green-400/10" : "border-black bg-green-200")}`}>
                                            <div className="flex justify-between items-center mb-2"><span className="text-[10px] uppercase font-black tracking-widest flex items-center gap-2">{taxGap > 0.05 ? <AlertCircle size={14} /> : taxGap < -0.05 ? <X size={14} /> : <CheckCircle size={14} />}{taxGap > 0.05 ? "TAX / SERVICE GAP" : taxGap < -0.05 ? "OVER-ALLOCATED!" : "EXACT MATCH!"}</span><span className="font-mono font-black text-lg">{taxGap < 0 ? "-" : ""}{currency}{Math.abs(taxGap).toFixed(2)}</span></div>
                                            {(taxGap > 0.05 || parseFloat(discountFee) > 0) && (
                                                <div className={`space-y-2 mt-3 pt-3 border-t-2 border-current ${darkMode ? "border-opacity-30" : "border-black"}`}>
                                                    {taxGap > 0.05 && (<div className="flex items-center justify-between"><span className="text-[9px] font-bold opacity-70">Split Tax:</span><div className="flex gap-1"><button onClick={() => setTaxMethod("PROPORTIONAL")} className={`text-[9px] font-bold px-2 py-1 rounded border transition-all ${taxMethod === "PROPORTIONAL" ? (darkMode ? "bg-blue-400 text-black border-blue-400" : "bg-black text-white border-black") : "border-current opacity-50 hover:opacity-100"}`}>% SPENT</button><button onClick={() => setTaxMethod("EQUAL_SPLIT")} className={`text-[9px] font-bold px-2 py-1 rounded border transition-all ${taxMethod === "EQUAL_SPLIT" ? (darkMode ? "bg-blue-400 text-black border-blue-400" : "bg-black text-white border-black") : "border-current opacity-50 hover:opacity-100"}`}>EQUAL</button></div></div>)}
                                                    {parseFloat(discountFee) > 0 && (<div className="flex items-center justify-between"><span className="text-[9px] font-bold opacity-70">Split Discount:</span><div className="flex gap-1"><button onClick={() => setDiscountMethod("PROPORTIONAL")} className={`text-[9px] font-bold px-2 py-1 rounded border transition-all ${discountMethod === "PROPORTIONAL" ? (darkMode ? "bg-green-400 text-black border-green-400" : "bg-black text-white border-black") : "border-current opacity-50 hover:opacity-100"}`}>% SPENT</button><button onClick={() => setDiscountMethod("EQUAL_SPLIT")} className={`text-[9px] font-bold px-2 py-1 rounded border transition-all ${discountMethod === "EQUAL_SPLIT" ? (darkMode ? "bg-green-400 text-black border-green-400" : "bg-black text-white border-black") : "border-current opacity-50 hover:opacity-100"}`}>EQUAL</button></div></div>)}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                )}                    </div>
                            <button disabled={!billTitle || !billTotal || (billType === "ITEMIZED" && taxGap < -0.1)} onClick={saveBill} className={`w-full py-5 rounded-xl text-sm font-black uppercase tracking-widest border-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none ${darkMode ? "bg-white text-black border-white hover:bg-gray-200" : "bg-lime-300 text-black border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[4px] hover:translate-y-[4px]"}`}>{editingBillId ? "UPDATE BILL NOW" : "SAVE THIS BILL"}</button>
                        </div>
                    )}
                    ...
                    {showPayModal && activeTransfer && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
                            <div className={`w-full max-w-[340px] max-h-[85vh] flex flex-col rounded-2xl overflow-hidden ${darkMode ? "bg-[#1E1E1E] text-white" : "bg-white text-black"} shadow-2xl relative animate-in zoom-in-95`}>
                                <div className="p-4 flex-shrink-0 flex justify-between items-center border-b border-current border-opacity-10"><span className="text-[10px] font-black uppercase tracking-widest opacity-50">READY TO SHARE</span><button onClick={() => setShowPayModal(false)}><X size={20} className="opacity-50 hover:opacity-100" /></button></div>
                                    ...
                                    <div ref={receiptRef} className={`w-full ${darkMode ? "bg-[#222]" : "bg-white"} p-3 rounded-2xl ${darkMode ? "border-white" : "border-black"} border-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden`}>
                                        <div className={`rounded-xl border-2 border-dashed ${darkMode ? "border-white/30 bg-white/5" : "border-black/20 bg-gray-50"} p-4 relative`}>
                                            ...
                                            <div className="text-center border-b-2 border-dashed border-current/20 pb-3 mb-3">
                                                <p className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1">TOTAL PAYMENT</p>
                                                <h2 className="text-3xl font-black font-mono tracking-tight">{currency}{activeTransfer.amount.toFixed(2)}</h2>
                                            </div>
                                            <div className="flex justify-between items-end mb-4">
                                                <div className="text-left">
                                                    <p className="text-[8px] font-bold opacity-40 uppercase mb-0.5">TO</p>
                                                ...
                                                <div className="text-right">
                                                    <p className="text-[8px] font-bold opacity-40 uppercase mb-0.5">FROM</p>
                                            ...
                                            <div className={`p-2 rounded border ${darkMode ? "border-white/20 bg-black/30" : "border-black/10 bg-white"} flex justify-between items-center mb-4`}>
                                                ...
                                                <span className="font-mono font-black text-sm tracking-wider">{people.find(p => p.id === activeTransfer.toId)?.bankAccount || "ASK MEMBER"}</span>
                                            </div>
                                            <div className="mb-4">
                                                <p className="text-[8px] font-bold uppercase opacity-40 mb-1 flex items-center gap-1"><List size={10} /> FOR:</p>
                                                <div className="flex flex-wrap gap-1">
                                                    ...
                                                    {getTransferDetails(activeTransfer.fromId, activeTransfer.toId).length > 3 && (
                                                        <span className={`text-[9px] font-bold px-1.5 py-0.5 border rounded opacity-60 ${darkMode ? "border-white/20 bg-white/5" : "border-black/20 bg-gray-100"}`}>+{getTransferDetails(activeTransfer.fromId, activeTransfer.toId).length - 3} more...</span>
                                                    )}
                                                    {getTransferDetails(activeTransfer.fromId, activeTransfer.toId).length === 0 && <span className="text-[9px] italic opacity-50">Account balance settlement...</span>}
                                                </div>
                                            </div>
                            ...
                            <div className={`w-full max-w-[360px] max-h-[85vh] flex flex-col rounded-2xl border-2 ${darkMode ? "bg-[#1E1E1E] border-white text-white" : "bg-white border-black text-black"} shadow-2xl relative animate-in zoom-in-95`}>
                                {isScanning ? (
                                    <div className="flex-1 flex flex-col items-center justify-center p-10 text-center space-y-4">
                                        ...
                                        <div><h3 className="text-xl font-black uppercase">Scanning Receipt...</h3><p className="text-xs font-bold opacity-60 mt-1">{scanStatus}</p></div>
                                    </div>
                                ) : (
                                    <>
                                        <div className="p-4 border-b border-current border-opacity-10 flex justify-between items-center">
                                            <h3 className="text-sm font-black uppercase flex items-center gap-2"><Camera size={16} /> AI Scan Result</h3>
                                                    ...
                                                    <div>
                                                        <p className="text-[10px] font-black uppercase tracking-wider opacity-70">
                                                            {scannedExtraInfo.tax > 0 || scannedExtraInfo.service > 0 
                                                                ? "Receipt Total (Tax/SC Included)" 
                                                                : "Receipt Total (Standard)"}
                                                        </p>
                                                        <p className="text-[9px] font-bold opacity-60">
                                                            {scannedExtraInfo.tax > 0 || scannedExtraInfo.service > 0 
                                                                ? `Tax/Service: ${currency}${(scannedExtraInfo.tax + scannedExtraInfo.service).toFixed(2)} (Auto-distributed)` 
                                                                : "No tax/service charges detected"}
                                                        </p>
                                                    </div>
                                                </div>
                                            ...
                                            {(scannedExtraInfo.discount > 0 || scannedExtraInfo.deposit > 0) && (
                                                <div className={`p-2 rounded-xl border-2 border-dashed flex items-start gap-2 ${darkMode ? "border-green-500/50 bg-green-500/10" : "border-green-600/30 bg-green-50"}`}>
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <label className="flex items-center gap-1.5 text-[9px] font-bold cursor-pointer"><input type="checkbox" checked={includeScannedDiscount} onChange={(e) => setIncludeScannedDiscount(e.target.checked)} className="accent-green-500 w-3 h-3" />Add to Discount box?</label>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                            {scannedItems.length === 0 ? (<div className="text-center py-10 opacity-50"><p className="text-xs font-bold">No items detected. Try scanning again.</p></div>) : (scannedItems.map(item => (<div key={item.id} className={`p-3 rounded-xl border-2 transition-all ${item.selected ? (darkMode ? "border-green-400 bg-green-400/5" : "border-green-500 bg-green-50/30") : "opacity-40 grayscale"}`}><div className="flex items-start gap-2 mb-3"><input type="checkbox" checked={item.selected} onChange={() => toggleScanItem(item.id)} className="mt-1 accent-green-500 w-4 h-4" /><div className="flex-1"><input value={item.name} onChange={e => updateScannedItem(item.id, 'name', e.target.value)} className="w-full bg-transparent font-bold text-xs outline-none border-b border-dashed border-current/20 focus:border-green-500 mb-1" /><div className="flex items-center gap-1"><span className="text-[10px] opacity-50">{currency}</span><input type="number" value={item.price} onChange={e => updateScannedItem(item.id, 'price', e.target.value)} className="w-20 bg-transparent font-mono font-black text-sm outline-none" /></div></div><button onClick={() => deleteScannedItem(item.id)} className="p-1 text-red-500 hover:bg-red-500/10 rounded-lg"><Trash2 size={14} /></button></div><div className="flex flex-wrap items-center gap-1 pt-2 border-t border-dashed border-current/10"><span className="text-[9px] font-bold opacity-50 mr-1">Shared by:</span>{people.map(p => { const isAssigned = item.sharedBy.includes(p.id); return (<button key={p.id} onClick={() => togglePersonInScan(item.id, p.id)} className={`px-2 py-1 rounded-md text-[9px] font-black uppercase border transition-all ${isAssigned ? (darkMode ? "bg-blue-500 border-blue-400 text-white" : "bg-blue-600 border-black text-white shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]") : (darkMode ? "border-white/20 text-white/40 hover:text-white" : "border-black/20 text-black/40 hover:text-black")}`}>{p.name}</button>); })}</div></div>)))}
                                        </div>
                                        <div className="p-4 border-t border-current border-opacity-10 bg-current/5">
                                            <button onClick={addSelectedScannedItems} className={`w-full py-3 rounded-xl font-black uppercase text-xs border-2 ${darkMode ? "bg-white text-black border-white" : "bg-black text-white border-black"} shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all`}>Add {scannedItems.filter(i => i.selected).length} Items to Bill</button>
                                        </div>
                                    </>
                                )}
                            </div>
                    ...
                    {showCurrencyModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
                            <div className={`w-full max-w-[320px] p-6 rounded-[2rem] border-2 ${darkMode ? "bg-zinc-900 border-white text-white" : "bg-white border-black text-black"} shadow-2xl relative`}>
                                ...
                                <h2 className="text-xl font-black uppercase mb-6 flex items-center gap-2 tracking-tighter"><Globe size={24} /> Select Currency</h2>
                                <div className="grid grid-cols-2 gap-3">
                    ...
                    {showSessionModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
                            <div className={`w-full max-w-[340px] p-6 rounded-2xl border-2 ${darkMode ? "bg-[#1E1E1E] border-white text-white" : "bg-white border-black text-black"} ${shadowStyle} relative`}>
                                ...
                                <h2 className="text-xl font-black uppercase mb-6 flex items-center gap-2"><Folder size={24} /> Select Event</h2>
                                    ...
                                    {sessions.map(s => (
                                        <div key={s.id} className={`p-4 rounded-xl border-2 flex items-center justify-between transition-all ${activeSessionId === s.id ? (darkMode ? "border-green-400 bg-green-900/20" : "border-black bg-green-100") : (darkMode ? "border-white/10 bg-white/5" : "border-black/5 bg-gray-50")}`}>
                                            {editingSessionId === s.id ? (
                                                ...
                                                <div onClick={() => { setActiveSessionId(s.id); setShowSessionModal(false); }} className="flex-1 cursor-pointer"><h3 className={`font-bold text-sm ${darkMode ? "text-white" : "text-black"}`}>{s.name}</h3><p className="text-[10px] opacity-50">{new Date(s.createdAt).toLocaleDateString()}{s.isShared ? " · Shared event" : ""}</p></div>
                                            )}
                                            <div className="flex items-center gap-1 pl-2">
                                                {activeSessionId === s.id && !editingSessionId && <CheckCircle size={16} className="text-green-500 mr-1" />}
                                                {!editingSessionId && (<><button onClick={() => startRenameSession(s)} aria-label={`Rename event ${s.name}`} title="Rename event" className={`p-2 rounded-lg transition-all ${darkMode ? "text-white/50 hover:bg-white/10 hover:text-blue-400" : "text-black/50 hover:bg-blue-100 hover:text-blue-700"}`}><Edit3 size={14} /></button><button onClick={() => deleteSession(s.id)} disabled={deletingSessionId !== null} aria-label={s.isShared ? `Leave event ${s.name}` : `Delete event ${s.name}`} title={s.isShared ? "Leave event" : "Delete event"} className={`p-2 rounded-lg border transition-all disabled:cursor-wait disabled:opacity-40 ${darkMode ? "border-red-400/30 bg-red-400/10 text-red-300 hover:border-red-300 hover:bg-red-400/20" : "border-red-200 bg-red-50 text-red-700 hover:border-red-500 hover:bg-red-100"}`}>{deletingSessionId === s.id ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}</button></>)}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                <div className="pt-6 border-t border-dashed border-current border-opacity-30">
                                    <label className="text-[10px] font-bold uppercase opacity-70 block mb-2">Create New Event</label>
                                    <div className="flex gap-2"><input value={newSessionName} onChange={e => setNewSessionName(e.target.value)} placeholder="e.g. Dinner with Friends" className={`flex-1 px-3 py-2 rounded-lg bg-transparent border-2 outline-none text-sm font-bold ${darkMode ? "border-white/30 focus:border-white" : "border-black/30 focus:border-black"}`} /><button onClick={createNewSession} disabled={!newSessionName} className={`px-4 py-2 rounded-lg border-2 font-bold text-sm ${darkMode ? "bg-white text-black border-white" : "bg-black text-white border-black"} disabled:opacity-50`}>OK</button></div>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {showPreviewModal && previewImage && (
                        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md animate-in fade-in">
                            <div className="relative w-full max-w-sm flex flex-col items-center gap-4 animate-in zoom-in-95 duration-300">
                                ...
                                <h3 className="text-white text-xs font-bold uppercase tracking-[0.2em] text-center opacity-80">Receipt Preview</h3>
                                <img src={previewImage} alt="Receipt Preview" className="w-full rounded-2xl border-2 border-white/20 shadow-2xl" />
                                <div className="text-center space-y-3 mt-2">
                                    <div className="bg-white/10 px-4 py-2 rounded-lg backdrop-blur"><p className="text-white text-[10px] font-bold uppercase tracking-wide animate-pulse">📲 iPhone: Long-press image</p><p className="text-white/50 text-[9px]">Select “Save to Photos” or “Share”</p></div>
                                    {typeof navigator !== "undefined" && navigator.share && (<button onClick={async () => { try { const blob = await (await fetch(previewImage)).blob(); const file = new File([blob], "Settlement.png", { type: "image/png" }); await navigator.share({ files: [file], title: 'SplitIt Receipt' }); } catch { } }} className="flex items-center gap-2 px-5 py-2 bg-blue-600 text-white font-bold rounded-full text-[10px] uppercase tracking-widest hover:bg-blue-500 transition-colors mx-auto"><ExternalLink size={12} /> Share Now</button>)}
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
                            <div className={`w-full max-w-[340px] p-6 rounded-2xl border-2 ${darkMode ? "bg-[#1E1E1E] border-white text-white" : "bg-white border-black text-black"} shadow-2xl relative animate-in zoom-in-95`}>
                                ...
                                <div className="text-center mb-6">
                                    ...
                                    <h2 className="text-xl font-black uppercase leading-tight">Invite Friends to Join!</h2>
                                    <p className="text-xs font-bold opacity-60 mt-2 leading-relaxed">
                                        Share this link with your group. Friends can join in real-time, add items, and select what they ate.
                                    </p>
                                </div>
                                ...
                                <button onClick={() => {
                                    ...
                                    if (typeof navigator !== "undefined" && navigator.share) {
                                        navigator.share({ title: 'Split Bill Together!', text: `Let's settle the bill for ${activeSession?.name} here:`, url: link }).catch(console.error);
                                    } else {
                                        navigator.clipboard.writeText(link);
                                        alert("Link copied to clipboard! Paste into chat or WhatsApp.");
                                    }
