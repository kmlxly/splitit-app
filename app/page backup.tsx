...
import { 
  ...
  CreditCard, QrCode, Upload, Wallet, ExternalLink, ArrowRight, Info, Folder, Calculator, Save, ShoppingBag, User, Globe
} from "lucide-react";
                            ...
                            <div className="space-y-6 animate-in fade-in">
                                <div className={`${cardStyle} p-5 space-y-4 ${darkMode ? "bg-[#1E1E1E]" : "bg-white"} ${shadowStyle} overflow-hidden transition-all duration-300`}>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <div className="flex-1 relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold opacity-50">{currency}</span><input type="number" value={newItemPrice} onChange={e => setNewItemPrice(e.target.value)} placeholder="0.00" className={`${inputStyle} pl-8 text-sm`}/></div>
                                    </div>
                                ...
                                <div className={`p-5 border-2 rounded-xl ${shadowStyle} ${taxGap > 0.05 ? (darkMode ? "border-blue-400 bg-blue-400/10" : "border-black bg-blue-200") : taxGap < -0.05 ? (darkMode ? "border-red-400 bg-red-400/10" : "border-black bg-red-200") : (darkMode ? "border-green-400 bg-green-400/10" : "border-black bg-green-200")}`}>
                                    <div className="flex justify-between items-center mb-3"><span className="text-xs uppercase font-black tracking-widest flex items-center gap-2">{taxGap > 0.05 ? <AlertCircle size={16}/> : taxGap < -0.05 ? <X size={16}/> : <CheckCircle size={16}/>}{taxGap > 0.05 ? "BAKI (TAX/SERVIS)" : taxGap < -0.05 ? "TERLEBIH KIRA!" : "NGAM-NGAM!"}</span><span className="font-mono font-black text-xl">{taxGap < 0 ? "-" : ""}{currency}{Math.abs(taxGap).toFixed(2)}</span></div>
